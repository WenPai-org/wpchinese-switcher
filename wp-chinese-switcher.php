...
if( $wpcs_options != false && is_array($wpcs_options) && is_array($wpcs_options['wpcs_used_langs']) ) {
    add_action('widgets_init', function ()  {return register_widget('wpcs_Widget'); }, 1 );
...
class wpcs_Widget extends WP_Widget {
	function __construct() {
		parent::__construct('widget_wpcs', 'Chinese Switcher', array('classname' => 'widget_wpcs', 'description' =>'Chinese Switcher Widget'));
	}
...
function wpcs_no_conversion_filter($str) {
	...
	if( !empty($wpcs_options['wpcs_no_conversion_tag']) ) {
		...
		if( preg_match('/^[a-z1-9|]+$/', $wpcs_options['wpcs_no_conversion_tag']) )
			$query .= str_replace('|', ',', $wpcs_options['wpcs_no_conversion_tag']); // backward compatability
		else
...
function wpcs_pagenum_link_fix($link) {
	...
	global $paged;
	if( $wpcs_options['wpcs_use_permalink'] != 1 ) return $link;
...
function wpcs_output_navi($args = '') {
	...
	else $default_url = $wpcs_noconversion_url;
...
function wpcs_parse_query($query) {
	...
	if(!$wpcs_target_lang) {
		if( $request_lang == 'zh' ) {
			...
			header('Location: ' . $wpcs_noconversion_url); die();
		}
